	...
	api.on('connection', function(socket){		
		...
		function check_token(t){
			...
			function handleSession(uid,name,avatar, callback){
				mysql_connection.query("select * from user_data where user_id = '"+uid+"' limit 1", function(err,res){
					...
					var u_data = JSON.parse(res[0].data);
					var rooms = u_data.rooms;
					var d_rooms = [];
					var user_data = res[0];
					user_data.rooms = [];
					for (var i = 0; i < rooms.length; i++)
					{
						mysql_connection.query("select * from rooms where name like '%"+rooms[i]+"%' limit 1 ", function(err, res){							
							if (!err && res[0] !== undefined)
							{								
								user_data.rooms.push(res[0]);								
							}							
							if (i == rooms.length) {							    
								callback(uid,name,avatar,user_data);
							}
						});
					}
				});
		...
		function getLastMessages(room, offset, limit){			
				...
				if(room.indexOf("->") != -1)
					...
					mysql_connection.query("select *, r.id as roomId from message as m inner join message_send as ms on m.id=ms.msg_id left join rooms as r on m.entry like concat('im',r.name,'') where m.entry='im"+room+"' and ms.isSender='1' order by date limit "+offset+","+limit, function(err, messages){
						if(!err){
							var msgs = [];
							if(!messages)
								var messages = [];
			...
			if(data.room == undefined || data.msg == undefined){
				...
				if(user_data == null){
					...
					if(user_data.rooms.indexOf(room) == -1){
						...
						// Write to BD	
						var onUpdateRoom = function(roomData)
						{							
							var sendBack = function(api, room_prefix, data, user_data, msg_id, cacheKey){
								var date = new Date();
								api.to(room_prefix+data.room).emit('get_messages', [{name:user_data.name, avatar:user_data.avatar, uid:user_data.uid, msg:data.msg, room:room, roomId: roomData.id, id:msg_id, date:date, cacheKey:cacheKey}]);
							}
							var sendToApp = function(message){							
								io.of("/general").emit("send_lhs_message", message);
								var date = new Date();
								var room = message.entry.replace("im", "");
								api.to(room_prefix+room).emit("get_messages", [{name:message.sender.uname, avatar:message.sender.uavatar, uid:message.sender.uid, msg:message.msg, room:room, roomId: roomData.id, id:message.id, date:date, cacheKey:message.cacheKey}]);
							}
						...
						updateRoomInDatabase(room, {uid:user_data.uid}, onUpdateRoom);
		...
		socket.on('kick_from_room', function(data){
			if(data.uid !== undefined && data.room !== undefined){				
				if(socket.session !== undefined && socket.session.user_data !== undefined){
					var sender = socket.session.user_data;
					var onGettingRoomCreator = function(resp){
						if(resp.length){
							var l_socket = findSocketByUid(data.uid);							
							if(l_socket !== undefined && l_socket){
								//console.log(io.sockets);
								l_socket.disconnect();
								deleteRoomUserData(data.uid, data.room);
							}							
						}
					};
					getRoomFromDatabase(data.room, sender, onGettingRoomCreator);
				}
			}
		});

		function deleteRoomUserData(uid, room){
			mysql_connection.query('select * from user_data where user_id='+uid, function(err,resp){
				if(err){
					console.log(err);
					return false;
				}
				if(resp[0] !== undefined){
					var data = resp[0];					
					var parsedRooms = JSON.parse(data.data);
					var rooms = parsedRooms.rooms;
					if(rooms.indexOf(room) != -1){
						var ind = rooms.indexOf(room);						
						delete rooms[ind];
						rooms.splice(ind, 1);		
						var s_rooms = {};				
						s_rooms.rooms = rooms;
						var s_data = JSON.stringify(s_rooms);
						mysql_connection.query("update user_data set data='"+s_data+"' where user_id="+uid);
					}
				}
			});
		}
		...
		function updateRoomInDatabase(room, sender, callback){
			mysql_connection.query('select * from rooms where name like "%'+room+'%" and creator='+sender.uid, function(err, resp){
				if(err){
					console.log(err);
					return false;
				}
				var roomData = null;
				if(!resp.length){
					var data = {name:room, creator:sender.uid};
					mysql_connection.query('insert into rooms set ? ', data, function(err, result){
						if (!err)
						{
							if (callback)
							{
		...
		function getRoomFromDatabase(room, sender, callback){
			mysql_connection.query('select * from rooms where name like "%'+room+'%" and creator='+sender.uid, function(err, resp){
				if(err){
					console.log(err);
					return false;
		...
		socket.on("get_room_data", function(){
			updateRoomInDatabase();
		});

		socket.on("vazcore_create_table", function(){
			createRoomTable();
		});

		function createRoomTable(){
			mysql_connection.query('create table rooms ( id int(11) not null auto_increment primary key, name varchar(255) not null, creator int(11) not null, data text null );');
		}
		...
		socket.on('invite_to_chat', function(data){
			...
			var onAfterRoomUpdate = function(roomData)
			{
				for (var i=0;i<uids.length;i++)
				{
					...
					{
						invite(uids[i], room, invite_id, roomData);
						handleUndoneThings('invites', 0, {room:room, roomId: roomData.id, sender:sender.uid, reciever:uids[i]});
					}				
				}
				
				api.to(room_prefix+sender.token).emit('notifications', {request_type:'invitation_send', status:'1', params:{room:room, roomId: roomData.id}, all_data:data});								
			};
			
			updateRoomInDatabase(room, sender, onAfterRoomUpdate);
			
			getLastMessages(room);

			function invite(uid, room, invite_id, roomData){
				var to_token = null;
				var to_name = null;	
				var from_name = null;		
				for(var i=0; i < sockets.length; i++){
				...
				var wc_data = {room:room, sender_id:sender.uid, type:"invite", all_data:data};
				...
				if(to_token == null){					
					...
					// Send invitaion
					api.to(room_prefix+to_token).emit('notifications', {request_type:'invite', status:'1', description:'New invitation was send', name: from_name, room:room, roomId:roomData.id,  invite_id:invite_id, all_data:data});								
				}
			}
